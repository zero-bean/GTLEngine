...
{
	if (UDOFComponent* DofComponent = Cast<UDOFComponent>(M.SourceObject); DofComponent == nullptr)
	{
		UE_LOG("DepthOfField: Source object is not a DOFComponent!\n");
		return;
	}
...
void FDepthOfFieldPass::ExecuteCoCPass(const FPostProcessModifier& M, FSceneView* View, D3D11RHI* RHIDevice)
{
	...
	UDOFComponent* DofComponent = Cast<UDOFComponent>(M.SourceObject);
