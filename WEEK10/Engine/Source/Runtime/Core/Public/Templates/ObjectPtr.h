...
public:
	...
	// 객체 포인터 생성자
	template <typename T>
	ObjectPtr(T* InObject) : Pointer(static_cast<void*>(InObject))
	{
	}
...
struct TObjectPtr
{
	...
	static_assert(std::is_base_of_v<UObject, T>, "TObjectPtr<T>는 UObject를 상속받은 타입만 사용할 수 있습니다");
...
public:
	...
	template <typename U>
	TObjectPtr(const TObjectPtr<U>& InOther) requires (std::is_convertible_v<U*, T*>) : ObjectPtr(InOther.ObjectPtr)
	{
	}
