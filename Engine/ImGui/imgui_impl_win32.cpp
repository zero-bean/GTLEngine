...
//  2022-01-12: Inputs: UpdatePhysics mouse inputs using WM_MOUSEMOVE/WM_MOUSELEAVE + fallback to provide it when focused but not hovered/captured. More standard and will allow us to pass it to future input queue API.
...
void    ImGui_ImplWin32_NewFrame()
{
    ...
    // UpdatePhysics OS mouse position
    ImGui_ImplWin32_UpdateMouseData(io);

    // Process workarounds for known Windows key handling issues
    ImGui_ImplWin32_ProcessKeyEventsWorkarounds(io);

    // UpdatePhysics OS mouse cursor with the cursor requested by imgui
    ...
    // UpdatePhysics game controllers (if enabled and available)
...
typedef DPI_AWARENESS_CONTEXT(WINAPI* PFN_SetThreadDpiAwarenessContext)(DPI_AWARENESS_CONTEXT); // User32.lib + dll, Windows 10 v1607+ (Creators UpdatePhysics)
